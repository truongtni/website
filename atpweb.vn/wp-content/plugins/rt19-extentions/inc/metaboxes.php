	...
	class rt_meta_boxes{
		...
		*/
		function __construct($settings,$customFields) {
			...
			add_action( 'save_post', array( &$this, 'saveCustomFields' ), 10 );					 
		}
		...
		*/
			...
			?>
			 <div class="rt-metaboxes">
				...
				foreach ( $this->customFields as $customField ) {
					...
					$help_icon = ! empty( $description ) ? '<span title="'.__('click for more information','rt_theme_admin').'" class="tooltip_icon icon-help-circled"></span>' : "";
					...
					if ( $output ) { ?>
							...
							switch ( $customField[ 'type' ] ) {
								...
								echo '<select autocomplete="off" name="'.$id.'" id="'.$id.'" class="'.$class.' '.$fontSystem.' '. $extraClass .' ">';
		...
		function saveCustomFields( $post_id ) {
				...
				if ( current_user_can( $this->settings['capability'], $post_id ) ) {
					...
					$value = isset( $_POST[ $this->prefix . str_replace("[]","", $customField_Name) ] ) ? $_POST[ $this->prefix . str_replace("[]","", $customField_Name) ] : "";				
						...
						if ( isset( $_POST[ $this->prefix . $customField_Name ] ) || isset( $_POST[ $this->prefix . str_replace("[]","", $customField_Name) ] )  ) {								
							...
							if( $customField[ 'default' ] != "" && empty( $value ) ){
								update_post_meta( $post_id, $this->prefix . $customField_Name, "" );				
							}else{
			...
			do_action("rt_updated_post_meta",$post_id);			
