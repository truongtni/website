...
if ( ! class_exists( 'rt_meta_box_gallery' ) ) :

	class rt_meta_box_gallery{
		#
		# @var $prefix
		# @var $customFields
		# @var $settings
			 ...
			 <div class="box right-col metaboxes side">
				<?php 
					...
					if ( $output ) { ?>  
							...
							<ul class="rt-gallery-uploaded-photos">
									...
									if( ! empty($rt_gallery_images) ){ 	
										...
										for ($i=0; $i < (count($rt_gallery_images_array)); $i++) {  
											...
											$image_id =  is_numeric( sanitize_file_name( $image_url ) ) ? sanitize_file_name( $image_url ) : rt_get_attachment_id_from_src( rt_clean_thumbnail_ext ( $image_url ) );
											...
											echo ' <li><img src="'.$thumbnail[0].'" data-rel="'.$image_url.'" width="114"></li> ';
							...
							<input type="hidden" autocomplete="off" name="<?php echo $this->prefix;?>rt_gallery_images" value="<?php echo $rt_gallery_images;?>" id="rt-gallery-images" class="upload_field">
