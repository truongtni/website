...
if( ! function_exists("rt_shortcode_contact_form") ){
	...
	$security_question['form_item'] = $security ? '<li class="security-question"><label class="math_label" for="math">'.__('Security Question:','rt_theme').'</label>'.$security_question['are_you_human_math1'].' + '.$security_question['are_you_human_math2'].' = <input id="math" placeholder="?" type="text" name="math" value="" class="required" /></li>' : "" ;
	...
	$contact_form .= '
		...
		<div class="clear"></div><div class="result"></div>
			<form action="#" name="contact_form" class="validate_form rt_form" method="post">
				<ul>
					<li class="form-input"><label for="name">'.__('Your Name: (*)','rt_theme').'</label><input placeholder="'.esc_attr(__('Your Name: (*)','rt_theme')).'" id="name" type="text" name="name" value="" class="required" /> </li>
					<li class="form-input"><label for="email">'.__('Your Email: (*)','rt_theme').'</label><input placeholder="'.esc_attr(__('Your Email: (*)','rt_theme')).'" id="email" type="email" name="email" value="" class="required email" /> </li>
					<li class="form-input"><label for="message">'.__('Your Message: (*)','rt_theme').'</label><textarea placeholder="'.esc_attr(__('Your Message: (*)','rt_theme')).'" id="message" name="message" rows="8" cols="40" class="required"></textarea></li>
					'.$security_question['form_item'].'
					<li class="submit-button">
					<input type="hidden" name="your_email" value="'.trim(base64_encode(sanitize_email($email))).'"><input type="hidden" name="dynamic_class" value="'.trim($dynamic_class).'"><input type="hidden" name="rt_form_data" value="'.base64_encode($security_question['are_you_human_sum']).'"><input type="button" class="button submit" value="'.esc_attr(__('Send','rt_theme')).'"  /><span class="loading"></span>
					</li>
				</ul>
			</form>
