 ...
 * @uses Walker
 */
class USER_Walker_Category_Multi extends Walker {
 ...
 * @since 0.8
 */
class USER_Walker_Category_Checklist extends Walker {
 ...
 */
function user_category_checklist( $post_id = 0, $selected_cats = false, $attr = array() ) {
    require_once ABSPATH . '/wp-admin/includes/template.php';

    $walker = new USER_Walker_Category_Checklist();
    ...
    echo '<ul class="user-category-checklist">';
 ...
 * @param type $post_id
 */
function user_associate_attachment( $attachment_id, $post_id ) {
 ...
 */
function user_get_avatar( $avatar, $id_or_email, $size, $default, $alt ) {
...
add_filter( 'get_avatar', 'user_get_avatar', 99, 5 );
...
function user_update_avatar( $user_id, $attachment_id ) {
    ...
    if ( function_exists( 'wp_get_image_editor' ) ) {
        ...
        if ( !is_wp_error( $editor ) ) {
            $avatar_size = apply_filters('user_avatar_size' , array( 100, 100 ), $user_id, $attachment_id);
 ...
 * @return string
 */
function user_show_custom_fields( $content ) {
    global $post;
    ...
    $show_custom = FRONTEND_USER()->helper->get_option( 'user-show-custom-meta', false );
    $form_id = FRONTEND_USER()->helper->get_option( 'user-submission-form', false );

    if (!$show_custom || !$form_id ) {
        return $content;
    }

    $html = '<ul class="user_customs">';
...
add_filter( 'the_content', 'user_show_custom_fields' );
 ...
 * @return type
 */
function user_get_attachment_id_from_url( $attachment_url = '', $author_id = 0 ) {
 ...
 * @since   2.2.2
*/
function user_allowed_html_tags() {
	$tags = array(
		'p' => array(
			'class' => array(),
            'style' => array()
	...
	return apply_filters( 'user_allowed_html_tags', $tags );
}

/**
 * Retrieves the currently displayed user
 *
 * This is used when display a user's store page
 *
 * This is a wrapper for USER_User_Shop::get_queried_user()
 *
 * @since 2.2.10
 * @return object|false WP User Object or false
 */
function user_get_user() {
    return FRONTEND_USER()->user_shop->get_queried_user();
}
