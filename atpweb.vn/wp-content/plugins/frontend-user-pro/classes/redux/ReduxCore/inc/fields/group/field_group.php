...
// Don't duplicate me!
if (!class_exists('ReduxFrameworkNew_group')) {

    /**
     * Main ReduxFrameworkNew_info class
     *
     * @since       1.0.0
     */
    class ReduxFrameworkNew_group {
        ...
        public function render() {
            ...
            echo '<div class="redux-groups-accordion-group redux-dummy" style="display:none" id="redux-dummy-' . $this->field['id'] . '"><h3><span class="redux-groups-header">' . __("New ", "redux-framework-new") . $this->field['groupname'] . '</span></h3>';
            ...
            //$field_is_title = true;
                ...
                echo '</table>';
                echo '<a href="javascript:void(0);" class="button deletion redux-groups-remove">' . __('Delete', 'redux-framework-new').' '. $this->field['groupname']. '</a>';
                echo '</div></div>';


            echo '</div><a href="javascript:void(0);" class="button redux-groups-add button-primary" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][slide_title][]">' . __('Add', 'redux-framework-new') .' '.$this->field['groupname']. '</a><br/>';
        ...
        public function enqueue() {
            wp_enqueue_script(
                ...
                ReduxFrameworkNew::$_url . 'inc/fields/group/field_group.js', 
            ...
            wp_enqueue_style(
                ...
                ReduxFrameworkNew::$_url . 'inc/fields/group/field_group.css', 
                time(), 
                true
            );
        }
        ...
        public function enqueue_dependencies($field_type) {
            $field_class = 'ReduxFrameworkNew_' . $field_type;

            if (!class_exists($field_class)) {
                $class_file = apply_filters('redux-typeclass-load', ReduxFrameworkNew::$_dir . 'inc/fields/' . $field_type . '/field_' . $field_type . '.php', $field_class);
