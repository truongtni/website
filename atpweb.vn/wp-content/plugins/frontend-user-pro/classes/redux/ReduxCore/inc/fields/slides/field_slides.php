...
// Don't duplicate me!
if (!class_exists('ReduxFrameworkNew_slides')) {

    /**
     * Main ReduxFrameworkNew_slides class
     *
     * @since       1.0.0
     */
    class ReduxFrameworkNew_slides {
        ...
        public function render() {
            ...
            if (isset($this->value) && is_array($this->value)) {
                ...
                foreach ($slides as $slide) {
                    ...
                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . __('Upload', 'redux-framework-new') . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide['attachment_id'] . '">' . __('Remove', 'redux-framework-new') . '</span>';

                    echo '</div>' . "\n";

                    echo '<ul id="' . $this->field['id'] . '-ul" class="redux-slides-list">';
                    $placeholder = (isset($this->field['placeholder']['title'])) ? esc_attr($this->field['placeholder']['title']) : __( 'Title', 'redux-framework-new' );
                    ...
                    $placeholder = (isset($this->field['placeholder']['description'])) ? esc_attr($this->field['placeholder']['description']) : __( 'Description', 'redux-framework-new' );
                    ...
                    $placeholder = (isset($this->field['placeholder']['url'])) ? esc_attr($this->field['placeholder']['url']) : __( 'URL', 'redux-framework-new' );
...
/*
                    if ( isset( $this->field['options'] ) && !empty( $this->field['options'] ) ) {
                        $placeholder = (isset($this->field['placeholder']['options'])) ? esc_attr($this->field['placeholder']['options']) : __( 'Select an Option', 'redux-framework-new' );
...
*/                    
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-slides-remove">' . __('Delete Slide', 'redux-framework-new') . '</a></li>';
            ...
            if ($x == 0) {
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][attachment_id]">' . __('Remove', 'redux-framework-new') . '</span>';
...
/*
                    if ( isset( $this->field['options'] ) && !empty( $this->field['options'] ) ) {
                        $placeholder = (isset($this->field['placeholder']['select'])) ? esc_attr($this->field['placeholder']['select']) : __( 'Select an Option', 'redux-framework-new' );
                        if ( isset( $this->field['select2'] ) ) { // if there are any let's pass them to js
                            $select2_params = json_encode( esc_attr( $this->field['select2'] ) );
                            $select2_params = htmlspecialchars( $select2_params , ENT_QUOTES);
                            echo '<input type="hidden" class="select2_params" value="'. $select2_params .'">';
...
*/
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . $this->field['id'] . '-ul" rel-name="' . $this->field['name'] . '[title][]">' . __('Add Slide', 'redux-framework-new') . '</a><br/>';
        ...
        public function enqueue() {
            ...
            wp_enqueue_script(
                ...
                ReduxFrameworkNew::$_url . 'inc/fields/media/field_media.js',
            ...
            wp_enqueue_style(
                ...
                ReduxFrameworkNew::$_url . 'inc/fields/media/field_media.css',
                time(),
                true
            );            
            ...
            wp_enqueue_script(
                ...
                ReduxFrameworkNew::$_url . 'inc/fields/slides/field_slides.js',
            ...
            wp_enqueue_style(
                ...
                ReduxFrameworkNew::$_url . 'inc/fields/slides/field_slides.css',
                time(),
                true
            );
