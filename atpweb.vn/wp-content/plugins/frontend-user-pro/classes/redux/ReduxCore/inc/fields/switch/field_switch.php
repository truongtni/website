<?php
class ReduxFrameworkNew_switch {	
	...
	*/
	function render(){
        ...
        if(!isset($this->field['off'])){
            $off = __('Off','redux-framework-new');
        } else{
            $off = $this->field['off'];
        }
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			...
			ReduxFrameworkNew::$_url.'inc/fields/switch/field_switch.js', 
			array('jquery'),
			time(),
			true
		);		
		...
		wp_enqueue_style(
			...
			ReduxFrameworkNew::$_url.'inc/fields/switch/field_switch.css', 
			time(),
			true
		);		
