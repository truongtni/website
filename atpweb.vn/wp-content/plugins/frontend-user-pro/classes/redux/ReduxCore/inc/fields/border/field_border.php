...
// Don't duplicate me!
if( class_exists( 'ReduxFrameworkNew_border' ) ) return;

class ReduxFrameworkNew_border {	
	...
	*/
	function render(){
		...
		if ( isset( $this->field['all'] ) && $this->field['all'] == true ) {
			echo '<div class="field-border-input input-prepend"><span class="add-on"><i class="el-icon-fullscreen icon-large"></i></span><input type="text" class="redux-border-all redux-border-input mini'.$this->field['class'].'" placeholder="'.__('All','redux-framework-new').'" rel="'.$this->field['id'].'-all" value="'.$this->value['top'].'"></div>';
		}
		...
		if ( !isset( $this->field['all'] ) || $this->field['all'] !== true ) :
			...
			**/
			if ($this->field['top'] === true):
				echo '<div class="field-border-input input-prepend"><span class="add-on"><i class="el-icon-arrow-up icon-large"></i></span><input type="text" class="redux-border-top redux-border-input mini'.$this->field['class'].'" placeholder="'.__('Top','redux-framework-new').'" rel="'.$this->field['id'].'-top" value="'.$this->value['top'].'"></div>';
		  	endif;
			...
			**/
			if ($this->field['right'] === true):
				echo '<div class="field-border-input input-prepend"><span class="add-on"><i class="el-icon-arrow-right icon-large"></i></span><input type="text" class="redux-border-right redux-border-input mini'.$this->field['class'].'" placeholder="'.__('Right','redux-framework-new').'" rel="'.$this->field['id'].'-right" value="'.$this->value['right'].'"></div>';
		  	endif;
			...
			**/
			if ($this->field['bottom'] === true):
				echo '<div class="field-border-input input-prepend"><span class="add-on"><i class="el-icon-arrow-down icon-large"></i></span><input type="text" class="redux-border-bottom redux-border-input mini'.$this->field['class'].'" placeholder="'.__('Bottom','redux-framework-new').'" rel="'.$this->field['id'].'-bottom" value="'.$this->value['bottom'].'"></div>';
		  	endif;
			...
			**/
			if ($this->field['left'] === true):
				echo '<div class="field-border-input input-prepend"><span class="add-on"><i class="el-icon-arrow-left icon-large"></i></span><input type="text" class="redux-border-left redux-border-input mini'.$this->field['class'].'" placeholder="'.__('Left','redux-framework-new').'" rel="'.$this->field['id'].'-left" value="'.$this->value['left'].'"></div>';
		  	endif;		
            ...
            **/
            ...
            if ( $this->field['style'] != false ):
                ...
                echo '<select original-title="' . __( 'Border style', 'redux-framework-new' ) . '" id="' . $this->field['id'] . '[border-style]" name="' . $this->parent->args['opt_name'] . '[' . $this->field['id'] . '][border-style]' . $this->field['name_suffix'] . '" class="tips redux-border-style' . $this->field['class'] . '" rows="6" data-id="'.$this->field['id'].'">';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script(
			...
			ReduxFrameworkNew::$_url.'inc/fields/border/field_border.js', 
			array('jquery', 'select2-js', 'redux-user'),
			time(),
			true
		);
		...
		wp_enqueue_style(
			...
			ReduxFrameworkNew::$_url.'inc/fields/border/field_border.css', 
			time(),
			true
		);	
