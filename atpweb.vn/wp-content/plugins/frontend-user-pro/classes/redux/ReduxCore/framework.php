 ...
 * @subpackage  Core
 * @author      Redux Framework New Team
 * @version     3.1.8
 */
...
if( !class_exists( 'ReduxFrameworkNew' ) ) {
    ...
    /**
     * Main ReduxFrameworkNew class
     *
     * @since       1.0.0
     */
    class ReduxFrameworkNew {
        ...
        static function init() {
            ...
            self::$_dir     = trailingslashit( Redux_Helpers_new::cleanFilePath( dirname( __FILE__ ) ) );
            $wp_content_dir = trailingslashit( Redux_Helpers_new::cleanFilePath( WP_CONTENT_DIR ) );
            $wp_content_dir = trailingslashit( str_replace( '//', '/', $wp_content_dir ) );
            $relative_url   = str_replace( $wp_content_dir, '', self::$_dir );
            $wp_content_url = Redux_Helpers_new::cleanFilePath( ( is_ssl() ? str_replace( 'http://', 'https://', WP_CONTENT_URL ) : WP_CONTENT_URL ) );
...
/**
        Still need to port these.
            ...
            $defaults['footer_credit']      = '<span id="footer-thankyou">' . __( 'Options panel created using', 'redux-framework-new') . ' <a href="' . $this->framework_url . '" target="_blank">' . __('Redux Framework New', 'redux-framework-new') . '</a> v' . self::$_version . '</span>';
            $defaults['help_tabs']          = array();
            $defaults['help_sidebar']       = ''; // __( '', 'redux-framework-new' );
...
**/
            ...
            self::$_properties = array(
                'args' => array(
                    ...
                    'menu_title' => array(
                            ...
                            'default'=> __( 'Options', 'redux-framework-new' )
                        ),
                    'page_title' => array(
                            'data_type'=>'string',
                            'label'=>'Page Title',
        ...
        public $args                = array(
            ...
            'page_slug'          => '_options',
            'page_permissions'   => 'manage_shop_settings',
            ...
            'help_tabs'          => array(),
            'help_sidebar'       => '', // __( '', 'redux-framework-new' );
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            if ( empty( $this->args['footer_credit'] ) ) {
                $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'redux-framework-new' ), '<a href="'.esc_url( $this->framework_url ).'" target="_blank">'.__( 'Redux Framework New', 'redux-framework-new' ).'</a> v'.self::$_version ) . '</span>';
            }
            if ( empty( $this->args['menu_title'] ) ) {
                $this->args['menu_title'] = __( 'Options', 'redux-framework-new' );
            }
            if ( empty( $this->args['page_title'] ) ) {
                $this->args['page_title'] = __( 'Options', 'redux-framework-new' );
            }

            /**
             * filter 'redux/args/{opt_name}'
             * @param  array $args  ReduxFrameworkNew configuration
             */
            ...
            if ( !empty( $this->args['opt_name'] ) ) {
                 ...
                 * @param object $this ReduxFrameworkNew
                 */
                ...
                // Add tracking_new. PLEASE leave this in tact! It helps us gain needed statistics of uses. Opt-in of course.
                //add_action( 'init', array( &$this, '_tracking_new' ), 200 );
                ...
                foreach( $this->admin_notices as $notice ) {
                    if (true == $notice['dismiss']) {
                        ...
                        if ( !get_user_meta( $userid, 'ignore_' . $notice['id'] ) ) {
                            ...
                            echo '<div class="' . $notice['type'] . '"><p>' . $notice['msg'] . '&nbsp;&nbsp;<a href="?dismiss=true&amp;id=' . $notice['id'] . $pageName . $curTab . '">' . __('Dismiss', 'redux-framework-new') . '</a>.</p></div>';
                        }
                    } else {
        ...
        public function _internationalization() {
             ...
             * @param string     'redux-framework-new'  text domain
             */
            $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'redux-framework-new' );
            ...
            load_textdomain( 'redux-framework-new', dirname( __FILE__ ) . '/languages/' . $locale . '.mo' );
        ...
        public function _tracking_new() {
            include_once( dirname( __FILE__ ) . '/inc/tracking_new.php' );
            new Redux_Tracking_new($this);
        } // _tracking_new()
...
//                $comparison = $field['required'][1];
            ...
            if( $this->args['menu_type'] == 'submenu' ) {
                ...
                if( true === $this->args['allow_sub_menu'] ) {
                    ...
                    if( true === $this->args['show_import_export'] ) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __( 'Import / Export', 'redux-framework-new' ),
                            __( 'Import / Export', 'redux-framework-new' ),
                            $this->args['page_permissions'],
                    ...
                    if( true === $this->args['dev_mode'] ) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __( 'Options Object', 'redux-framework-new' ),
                            __( 'Options Object', 'redux-framework-new' ),
                            $this->args['page_permissions'],
                    ...
                    if( true === $this->args['system_info'] ) {
                        add_submenu_page(
                            $this->args['page_slug'],
                            __( 'System Info', 'redux-framework-new' ),
                            __( 'System Info', 'redux-framework-new' ),
                            $this->args['page_permissions'],
        ...
        function _admin_bar_menu()
        {
            ...
            if($menu){
                ...
                $nodeargs = array(
                    ...
                    'title' => "<span class='ab-icon dashicons-admin-generic'></span>" . $theme_data->get('Name') . " " . __('Options', 'redux-framework-new-demo'),
        ...
        public function _enqueue_output() {
                ...
                if( isset( $section['fields'] ) ) {
                    ...
                    foreach( $section['fields'] as $fieldk => $field ) {
                        if( isset( $field['type'] ) && $field['type'] != "callback"  ) {
                            $field_class = "ReduxFrameworkNew_{$field['type']}";
            ...
            if ( !empty( $this->typography ) && !empty( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                ...
                $typography = new ReduxFrameworkNew_typography( null, null, $this );
        ...
        public function _enqueue() {
            ...
            wp_register_style(
                'redux-elusive-icon',
                self::$_url . 'assets/css/user/elusive-icons/elusive-webfont.css',
                array(),
                filemtime( self::$_dir . 'assets/css/user/elusive-icons/elusive-webfont.css' ),
                'all'
            );

            wp_register_style(
                'redux-elusive-icon-ie7',
                self::$_url . 'assets/css/user/elusive-icons/elusive-webfont-ie7.css',
                array(),
                filemtime( self::$_dir . 'assets/css/user/elusive-icons/elusive-webfont-ie7.css' ),
                'all'
            );

            wp_register_style(
                'select2-css',
                self::$_url . 'assets/js/user/select2/select2.css',
                array(),
                filemtime( self::$_dir . 'assets/js/user/select2/select2.css' ),
                'all'
            );

            wp_register_style(
                'qtip-css',
                self::$_url . 'assets/css/user/qtip/jquery.qtip.css',
                array(),
                filemtime( self::$_dir . 'assets/css/user/qtip/jquery.qtip.css' ),
                'all'
            );
            ...
            wp_register_style(
                'jquery-ui-css',
                apply_filters( "redux/page/{$this->args['opt_name']}/enqueue/jquery-ui-css", self::$_url . 'assets/css/user/jquery-ui-bootstrap/jquery-ui-1.10.0.custom.css' ),
                '',
                filemtime( self::$_dir . 'assets/css/user/jquery-ui-bootstrap/jquery-ui-1.10.0.custom.css' ), // todo - version should be based on above post-filter src
                'all'
            );
            ...
            if ( $this->args['dev_mode'] === true) { // Pretty object output
                ...
                wp_enqueue_script(
                    'json-view-js',
                    self::$_url . 'assets/js/user/jsonview.min.js',
                    array( 'jquery' ),
                    time(),
                    true
                );
            ...
            wp_register_script(
                'select2-js',
                self::$_url . 'assets/js/user/select2/select2.min.js',
                array( 'jquery' ),
                filemtime( self::$_dir . 'assets/js/user/select2/select2.min.js' ),
                true
            );

            wp_register_script(
                'qtip-js',
                self::$_url . 'assets/js/user/qtip/jquery.qtip.js',
                array( 'jquery' ),
                filemtime( self::$_dir . 'assets/js/user/qtip/jquery.qtip.js' ),
                true
            );

            wp_register_script(
                'ace-editor-js',
                self::$_url . 'assets/js/user/ace_editor/ace.js',
                array( 'jquery' ),
                filemtime( self::$_dir . 'assets/js/user/ace_editor/ace.js' ),
                true
            );
            ...
            if ( isset($this->args['dev_mode'] ) && $this->args['dev_mode'] === true) {
                wp_register_script(
                    'redux-user',
                    self::$_url . 'assets/js/user.min.js',
                    array( 'jquery'),
                    time(),
                    true
                );
                wp_register_script(
                    ...
                    array( 'jquery', 'select2-js', 'ace-editor-js',  'qtip-js', 'redux-user' ),
                    time(),
                    true
                );
            } else {
            ...
            $this->localize_data['args'] = array(
                'save_pending'          => __( 'You have changes that are not saved. Would you like to save them now?', 'redux-framework-new' ),
                'reset_confirm'         => __( 'Are you sure? Resetting will lose all custom values.', 'redux-framework-new' ),
                'reset_section_confirm' => __( 'Are you sure? Resetting will lose all custom values in this section.', 'redux-framework-new' ),
                'preset_confirm'        => __( 'Your current options will be replaced with the values of this preset. Would you like to proceed?', 'redux-framework-new' ),
        ...
        public function _load_page() {
            ...
            if (true == $this->show_hints) {
                ...
                $tab = array (
                    'id'        => 'redux-hint-tab',
                    'title'     => __('Hints', 'redux-framework-new-demo'),
                    'content'   => __('<p>' . $msg . '</p>', 'redux-framework-new-demo')
                );
                $screen->add_help_tab( $tab );
            }
            ...
            if( $this->args['help_sidebar'] != '' ) {
                ...
                // about hints.
                if (true == $this->show_hints) {
                    $screen->set_help_sidebar( '<p><strong>Redux Framework New</strong><br/><br/>Hint Tooltip Preferences</p>' );
                }
            }
        ...
        public function get_default_output_string($field) {
          ...
          if ( !empty( $default_output ) ) {
              $default_output = __( 'Default', 'redux-framework-new' ) . ": " . substr($default_output, 0, -2);
          }
          if (!empty($default_output)) {
        ...
        public function _register_extensions() {
            ...
            foreach($folders as $folder){
                ...
                $extension_class = 'ReduxFrameworkNew_Extension_' . $folder;
        ...
        public function _options_page_html() {
            ...
            // Do we support JS?
            echo '<noscript><div class="no-js">' . __( 'Warning- This options panel will not work properly without javascript!', 'redux-framework-new' ) . '</div></noscript>';
            ...
            if( !empty( $this->args['display_name'] ) ) {
                ...
                echo '<h2>' . $this->args['display_name'];
            ...
            echo '<a href="javascript:void(0);" class="expand_options">' . __( 'Expand', 'redux-framework-new' ) . '</a>';
            echo '<div class="redux-action_bar">';
            submit_button( __( 'Save Changes', 'redux-framework-new'), 'primary', 'redux_save', false );
            echo '&nbsp;';
            submit_button( __( 'Reset Section', 'redux-framework-new'), 'secondary', $this->args['opt_name'] . '[defaults-section]', false );
            echo '&nbsp;';
            submit_button( __( 'Reset All', 'redux-framework-new' ), 'secondary', $this->args['opt_name'] . '[defaults]', false );
            echo '</div>';

            echo '<div class="redux-ajax-loading" alt="' . __( 'Working...', 'redux-framework-new' ) . '">&nbsp;</div>';
            echo '<div class="clear"></div>';
            echo '</div>';
            ...
            if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] == 'true' && $saved == '1' ) {
                if( isset( $this->options['REDUX_imported'] ) && $this->options['REDUX_imported'] === 1 ) {
                     ...
                     */
                    echo '<div id="redux-imported"><strong>' . apply_filters( "redux-imported-text-{$this->args['opt_name']}", __( 'Settings Imported!', 'redux-framework-new' ) ) . '</strong></div>';
                } else {
                    /**
                     * filter 'redux-saved-text-{opt_name}'
                     * @param string translated "settings saved" text
                     */
                    echo '<div id="redux-save"><strong>' . apply_filters( "redux-saved-text-{$this->args['opt_name']}", __( 'Settings Saved!', 'redux-framework-new' ) ) . '</strong></div>';
                }
            }
            /**
             ...
             */
            echo '<div id="redux-save-warn"><strong>' . apply_filters( "redux-changed-text-{$this->args['opt_name']}", __( 'Settings have changed, you should save them!', 'redux-framework-new' ) ) . '</strong></div>';
            echo '<div id="redux-field-errors"><strong><span></span> ' . __( 'error(s) were found!', 'redux-framework-new' ) . '</strong></div>';
            echo '<div id="redux-field-warnings"><strong><span></span> ' . __( 'warning(s) were found!', 'redux-framework-new' ) . '</strong></div>';
            ...
            //     echo '<a href="javascript:void(0);" id="import_export_default_section_group_li_a" class="redux-group-tab-link-a" data-rel="import_export_default">' . $icon . ' <span class="group_title">' . __( 'Import / Export', 'redux-framework-new' ) . '</span></a>';
            //     echo '</li>';
            ...
            if( $this->args['dev_mode'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="dev_mode_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="dev_mode_default">' . $icon . ' <span class="group_title">' . __( 'Options Object', 'redux-framework-new' ) . '</span></a>';
                echo '</li>';
            }
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<a href="javascript:void(0);" id="system_info_default_section_group_li_a" class="redux-group-tab-link-a custom-tab" data-rel="system_info_default">' . $icon . ' <span class="group_title">' . __( 'System Info', 'redux-framework-new' ) . '</span></a>';
                echo '</li>';
            }
            ...
            if( $this->args['show_import_export'] === true ) {
                ...
                echo '<h3>' . __( 'Import / Export Options', 'redux-framework-new' ) . '</h3>';
                echo '<h4>' . __( 'Import Options', 'redux-framework-new' ) . '</h4>';
                echo '<p><a href="javascript:void(0);" id="redux-import-code-button" class="button-secondary">' . __( 'Import from file', 'redux-framework-new' ) . '</a> <a href="javascript:void(0);" id="redux-import-link-button" class="button-secondary">' . __( 'Import from URL', 'redux-framework-new' ) . '</a></p>';
                 ...
                 */
                echo '<p class="description" id="import-code-description">' . apply_filters( 'redux-import-file-description', __( 'Input your backup file below and hit Import to restore your sites options from a backup.', 'redux-framework-new' ) ) . '</p>';
                echo '</div>';
                 ...
                 * @param string translated settings import instructions text
                 */
                echo '<p class="description" id="import-link-description">' . apply_filters( 'redux-import-link-description', __( 'Input the URL to another sites options set and hit Import to load the options from that site.', 'redux-framework-new' ) ) . '</p>';
                echo '</div>';
                 ...
                 */
                echo '<p id="redux-import-action"><input type="submit" id="redux-import" name="' . $this->args['opt_name'] . '[import]" class="button-primary" value="' . __( 'Import', 'redux-framework-new' ) . '">&nbsp;&nbsp;<span>' . apply_filters( 'redux-import-warning', __( 'WARNING! This will overwrite all existing option values, please proceed with caution!', 'redux-framework-new' ) ) . '</span></p>';
                echo '<div class="hr"/><div class="inner"><span>&nbsp;</span></div></div>';

                echo '<h4>' . __( 'Export Options', 'redux-framework-new' ) . '</h4>';
                echo '<div class="redux-section-desc">';
                /**
                 * filter 'redux-backup-description'
                 * @param string translated settings backup information
                 */
                echo '<p class="description">' . apply_filters( 'redux-backup-description', __( 'Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'redux-framework-new' ) ) . '</p>';
                echo '</div>';


                /** @noinspection PhpUndefinedConstantInspection */
                echo '<p><a href="javascript:void(0);" id="redux-export-code-copy" class="button-secondary">' . __( 'Copy', 'redux-framework-new' ) . '</a> <a href="' . add_query_arg( array( 'feed' => 'redux_options_' . $this->args['opt_name'], 'action' => 'download_options', 'secret' => md5( AUTH_KEY . SECURE_AUTH_KEY ) ), site_url() ) . '" id="redux-export-code-dl" class="button-primary">' . __( 'Download', 'redux-framework-new' ) . '</a> <a href="javascript:void(0);" id="redux-export-link" class="button-secondary">' . __( 'Copy Link', 'redux-framework-new' ) . '</a></p>';
                $backup_options = $this->options;
                $backup_options['redux-backup'] = '1';
            ...
            if( $this->args['dev_mode'] === true ) {
                ...
                echo '<h3>' . __( 'Options Object', 'redux-framework-new' ) . '</h3>';
                echo '<div class="redux-section-desc">';
                ...
                echo '<a href="#" id="consolePrintObject" class="button">' . __( 'Show Object in Javascript Console Object', 'redux-framework-new' ) . '</a>';
            ...
            if( $this->args['system_info'] === true ) {
                ...
                echo '<h3>' . __( 'System Info', 'redux-framework-new' ) . '</h3>';
            ...
            echo '<div id="redux-footer-sticky">';
            
            echo '<div id="redux-footer">';
        ...
        public function _section_desc( $section ) {
            ...
            if( isset( $this->sections[$id]['desc'] ) && !empty( $this->sections[$id]['desc'] ) ) {
                echo '<div class="redux-section-desc">';
                if ($id == 3) {
                    echo "<div class='redux-preview'>";
                    echo '<p class="description">Button Preview</p>';
                    echo '<div>'.do_shortcode('[readlist_btn]').'</div>';
                    echo "</div>";
                }
...
//                    $class_file = apply_filters( 'redux/field/class/'.$field['type'], self::$_dir . 'inc/fields/' . $field['type'] . '/field_' . $field['type'] . '.php', $field ); // REMOVE
    ...
    do_action( 'redux/init', ReduxFrameworkNew::init() );
    //add_action( 'plugins_loaded', ReduxFrameworkNew::init(), 0 );

} // class_exists('ReduxFrameworkNew')
