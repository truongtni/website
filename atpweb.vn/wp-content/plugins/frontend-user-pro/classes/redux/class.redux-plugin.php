<?php
/**
 * ReduxFrameworkNew main class
 *
 * @package     ReduxFrameworkNew\ReduxFrameworkNew
 * @since       3.0.0
 */
...
if( !class_exists( 'ReduxFrameworkNew' ) ) {
    ...
    class ReduxFrameworkNew {
         ...
         * @since       3.0.0
         */
        protected $plugin_slug = 'redux-framework-new';
        ...
        /**
         * @access      private
         * @var         \ReduxFrameworkNew $instance The one true ReduxFrameworkNew
         * @since       3.0.0
         */
         ...
         * @access      public
         * @since       3.1.3
         * @return      self::$instance The one true ReduxFrameworkNew
         */
        ...
        public function get_redux_options() {
            ...
            if( is_multisite() ) {
                ...
                foreach( $plugins as $file => $plugin ) {
                    if( strpos( $file, 'redux-framework-new.php' ) !== false ) {
                        $this->plugin_network_activated = true;
                        $this->options = get_site_option( 'ReduxFrameworkNew', $defaults );
                    }
                }
            }
            ...
            if( empty( $this->options ) ) {
                $this->options = get_option( 'ReduxFrameworkNew', $defaults );
            }
        }
        ...
        public static function deactivate( $network_wide ) {
            ...
            delete_option( 'ReduxFrameworkNew' );
        }
        ...
        private static function single_activate() {
            $notices = get_option( 'ReduxFrameworkNew_ACTIVATED_NOTICES', array() );
            $notices[] = __( 'Redux Framework New has an embedded demo.', 'redux-framework-new' ) . ' <a href="./plugins.php?ReduxFrameworkNew=demo">' . __( 'Click here to activate the sample config file.', 'redux-framework-new' ) . '</a>';

            update_option( 'ReduxFrameworkNew_ACTIVATED_NOTICES', $notices );
        }
        ...
        public function admin_notices() {
            do_action( 'ReduxFrameworkNew_admin_notice' );

            if( $notices = get_option( 'ReduxFrameworkNew_ACTIVATED_NOTICES' ) ) {
                foreach( $notices as $notice ) {
                    echo '<div class="updated"><p>' . $notice . '</p></div>';
                }

                delete_option( 'ReduxFrameworkNew_ACTIVATED_NOTICES' );
            }
        }
        ...
        public function options_toggle_check() {
            ...
            if( $pagenow == 'plugins.php' && is_admin() && !empty( $_GET['ReduxFrameworkNew'] ) ) {
                $url = './plugins.php';

                if( $_GET['ReduxFrameworkNew'] == 'demo' ) {
                ...
                if( is_multisite() && is_network_admin() && $this->plugin_network_activated ) {
                    update_site_option( 'ReduxFrameworkNew', $this->options );
                } else {
                    update_option( 'ReduxFrameworkNew', $this->options );
                }
        ...
        public function add_action_links( $links ) {
            ...
            /**
             * return array_merge( 
             *      array( 'redux_plugin_settings' => '<a href="' . admin_url( 'plugins.php?page=' . 'redux_plugin_settings' ) . '">' . __( 'Settings', 'redux-framework-new' ) . '</a>' ), 
             *      $links
             * );
             */
        }
        ...
        public function plugin_metalinks( $links, $file ) {
            if( strpos($file,'redux-framework-new.php') !== false ) {
                ...
                $new_links = array(
                    '<a href="https://github.com/ReduxFrameworkNew/ReduxFrameworkNew" target="_blank">' . __( 'Github Repo', 'redux-framework-new' ) . '</a>',
                    '<a href="https://github.com/ReduxFrameworkNew/ReduxFrameworkNew/issues/" target="_blank">' . __( 'Issue Tracker', 'redux-framework-new' ) . '</a>'
                );

                if( ( is_multisite() && $this->plugin_network_activated ) || !is_network_admin() || !is_multisite() ) {
                    if( $this->options['demo'] ) {
                        $new_links[1] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkNew=demo" style="color: #bc0b0b;">' . __( 'Deactivate Demo Mode', 'redux-framework-new' ) . '</a></span>';
                    } else {
                        $new_links[1] .= '<br /><span style="display: block; padding-top: 6px;"><a href="./plugins.php?ReduxFrameworkNew=demo" style="color: #bc0b0b;">' . __( 'Activate Demo Mode', 'redux-framework-new' ) . '</a></span>';
                    }
                }
