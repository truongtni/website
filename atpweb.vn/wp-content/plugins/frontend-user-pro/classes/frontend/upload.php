...
class USER_Upload {
    ...
    function __construct() {
        ...
        add_action( 'wp_ajax_user_file_upload', array($this, 'upload_file') );
        add_action( 'wp_ajax_nopriv_user_file_upload', array($this, 'upload_file') );

        add_action( 'wp_ajax_user_file_del', array($this, 'delete_file') );
        add_action( 'wp_ajax_nopriv_user_file_del', array($this, 'delete_file') );

        add_action( 'wp_ajax_user_insert_image', array( $this, 'insert_image' ) );
        add_action( 'wp_ajax_nopriv_user_insert_image', array( $this, 'insert_image' ) );
    }
    ...
    function upload_file( $image_only = false ) {
        ...
        if ( $attach['success'] ) {
            ...
            if ($image_only) {
                $image_size = user_get_option( 'insert_photo_size', 'user_general', 'thumbnail' );
                $image_type = user_get_option( 'insert_photo_type', 'user_general', 'link' );
    ...
    public static function attach_html( $attach_id, $type = NULL ) {
        ...
        // echo "</pre>";
        if ( !$type ) {
            $type = isset( $_GET['type'] ) ? $_GET['type'] : 'image_upload';
        }
        ...
        $html .= sprintf( '<div class="caption"><a href="#" class="btn btn-danger btn-small attachment-delete" data-attach_id="%d">%s</a></div>', $attach_id, __( 'Delete', 'user' ) );
        if ($type == 'avatar') {
            $html .= sprintf( '<input type="hidden" name="avatar_id" class="user-avatar-image-id" value="%d" >',$attach_id);        

        }
        elseif( $type =='file_upload' ) {
            $html .= sprintf( '<input type="hidden" name="'.$type.'[]" class="user-file-image-id" value="%d" >',$attach_id);        
        }
        else{
            $html .= sprintf( '<input type="hidden" name="'.$type.'[]" class="user-upload-image-id" value="%d" >',$attach_id );
        }
    ...
    function delete_file() {
        check_ajax_referer( 'user_nonce', 'nonce' );
