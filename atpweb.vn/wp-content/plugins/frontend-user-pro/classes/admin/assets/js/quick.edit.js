( function( $ ) { 
	...
	inlineEditPost.edit = function( id ) {
		...
		if ( $post_id > 0 ) {
			...
			jQuery.each( uniqueNames, function( index, value ) {
				...
				// reset hidden fields
				$( '.feua-quick-edit .hidden-fields' ).html( '' );
				...
				if( form.is( ":checked" ) ) {
					...
					form.on( "click", function() {
						if( ! form.prop( "checked" ) ) {
							$( '.feua-quick-edit .hidden-fields' ).append( '<input type="hidden" name="remove-' + form.attr( 'id' ) + '" id="remove-' + form.attr( 'id' ) + '" value="on">' );
						} else {
	...
	$( ".inline-edit-feua_style .button-primary" ).on( "click", function() {
		var url = window.location.href;
		$( ".feua-quick-edit" ).load( url + " .feua-quick-edit >" );
	} );
