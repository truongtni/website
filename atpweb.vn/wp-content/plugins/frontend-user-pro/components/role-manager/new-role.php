...
if ( isset($_POST['new-role-submit']) && 'Y' == $_POST['new-role-submit'] ) {
	...
	/* Verify the nonce. */
	check_admin_referer( feup_members_get_nonce( 'new-role' ) );
...
<div class="wrap">
	...
	<h2><?php _e('Add a new user role', 'feup_members'); ?></h2>

	<?php if ( isset($new_role_added) and $new_role_added ) feup_members_admin_message( '', sprintf( __('The %1$s role has been created.', 'feup_members'), $_POST['role-name'] ) ); ?>

	<?php do_action( 'feup_members_pre_new_role_form' ); // Available action hook for displaying messages. ?>

	<div id="poststuff">

		<form name="form0" method="post" action="<?php echo admin_url( "users.php?page=new-role" ); ?>" style="border:none;background:transparent;">

			<?php wp_nonce_field( feup_members_get_nonce( 'new-role' ) ); ?>

			<div class="postbox open">

				<h3><?php _e('Create a new user role', 'feup_members'); ?></h3>
				...
				<div class="inside">
					...
					<tr>
						<th style="width: 20%;">
							<label for="role-name"><strong><?php _e('Role Label:', 'feup_members'); ?></strong></label>
						</th>
						<td>
							<?php _e('<strong>Required:</strong> Enter a label your role.  This will be the title that is displayed in most cases.', 'feup_members'); ?>
							<br />
					...
					<tr>
						<th style="width: 20%;">
							<strong><?php _e('Capabilities:', 'feup_members'); ?></strong>
						</th>
						<td>
							<?php _e('<strong>Optional:</strong> Select which capabilities your new role should have.  These may be changed later using the <em>Edit Roles</em> component.', 'feup_members'); ?>
							<br /><br />
							<?php foreach ( feup_members_get_capabilities() as $cap ) : ?>
								<div style="float: left; width: 32.67%; margin: 0 0 5px 0;">
									<input name='capabilities[<?php echo $cap; ?>]' id='capabilities-<?php echo $cap; ?>' type="checkbox" value='<?php echo $cap; ?>' <?php if ( in_array( $cap, feup_members_new_role_default_capabilities() ) ) echo "checked='checked'"; ?> /> 
									<label for="capabilities-<?php echo $cap; ?>"><?php if ( in_array( $cap, feup_members_new_role_default_capabilities() ) ) echo "<strong>$cap</strong>"; else echo $cap; ?></label>
								</div>
							<?php endforeach; ?>
						</td>
					</tr>
			...
			<p class="submit">
				<input type="submit" name="Submit" class="button-primary" value="<?php _e('Create Role', 'feup_members') ?>" />
