...
elseif ( 'delete' == $action && ( isset( $_POST['doaction'] ) && __('Apply', 'feup_members') == $_POST['doaction']) )
	$action = 'bulk-delete';

/* If the bulk delete (second submit) has been selected. */
elseif ( 'delete' == $action && ( isset($_POST['doaction2']) && __('Apply', 'feup_members') == $_POST['doaction2']) )
	$action = 'bulk-delete';
...
switch( $action ) {
	...
	case 'bulk-delete' :
		...
		/* If roles were selected, let's delete some roles. */
			...
			add_action( 'feup_members_pre_edit_roles_form', 'feup_members_message_roles_deleted' );
	...
	case 'delete' :
		...
		add_action( 'feup_members_pre_edit_roles_form', 'feup_members_message_role_deleted' );
	...
	case 'role-updated' :
		...
		$title = __('Edit Role', 'feup_members');
		$role = $_GET['role'];
