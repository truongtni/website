...
if ( 'active' == $role_status ) {	
	...
	$title = __('Edit Active Roles', 'feup_members');
...
elseif ( 'inactive' == $role_status ) {
	...
	$title = __('Edit Inactive Roles', 'feup_members');
...
/* Set default variables for when role_status is neither active nor inactive. */
	...
	$title = __('Edit Roles', 'feup_members');
...
<div class="wrap">
	...
	<div id="poststuff">
		...
		<form id="roles" action="<?php echo $current_page; ?>" method="post">
			...
			<ul class="subsubsub">
				<li><a <?php if ( 'all' == $role_status ) echo 'class="current"'; ?> href="<?php echo admin_url( esc_url( 'users.php?page=roles' ) ); ?>"><?php _e('All', 'feup_members'); ?> <span class="count">(<span id="all_count"><?php echo $all_roles; ?></span>)</span></a> | </li>
				<li><a <?php if ( 'active' == $role_status ) echo 'class="current"'; ?> href="<?php echo admin_url( esc_url( 'users.php?page=roles&amp;role_status=active' ) ); ?>"><?php _e('Active', 'feup_members'); ?> <span class="count">(<span id="active_count"><?php echo $active_roles; ?></span>)</span></a> | </li>
				<li><a <?php if ( 'inactive' == $role_status ) echo 'class="current"'; ?> href="<?php echo admin_url( esc_url( 'users.php?page=roles&amp;role_status=inactive' ) ); ?>"><?php _e('Inactive', 'feup_members'); ?> <span class="count">(<span id="inactive_count"><?php echo $inactive_roles; ?></span>)</span></a></li>
			</ul><!-- .subsubsub -->
			...
			<div class="tablenav">
				...
				<div class="alignleft actions">
					<select name="action">
						...
						<?php if ( $delete_roles ) echo '<option value="delete">' . __('Delete', 'feup_members') . '</option>'; ?>
					</select>
					<input type="submit" value="<?php _e('Apply', 'feup_members'); ?>" name="doaction" id="doaction" class="button-secondary action" />
			...
			<table class="widefat fixed" cellspacing="0">
				<thead>
					<tr>
						...
						<th class='name-column'><?php _e('Role Name', 'feup_members'); ?></th>
				...
				<?php foreach ( $roles_loop_array as $role => $name ) { ?>
					...
					<tr valign="top" class="<?php if ( isset($avail_roles[$role]) ) echo 'active'; else echo 'inactive'; ?>">
						...
						<td class="plugin-title">
							...
							<a href="<?php echo $edit_link; ?>" title="<?php printf( __('Edit the %1$s role', 'feup_members'), $name ); ?>"><strong><?php echo $name; ?></strong></a>

							<div class="row-actions">
								<a href="<?php echo $edit_link; ?>" title="<?php printf( __('Edit the %1$s role', 'feup_members'), $name ); ?>"><?php _e('Edit', 'feup_members'); ?></a> 

								<?php /* Delete role link. */
								if ( $delete_roles && $role !== $default_role && !$user->has_cap( $role ) ) {
									$delete_link = admin_url( wp_nonce_url( "users.php?page=roles&amp;action=delete&amp;role={$role}", feup_members_get_nonce( 'edit-roles' ) ) ); ?>
									| <a href="<?php echo $delete_link; ?>" title="<?php printf( __('Delete the %1$s role', 'feup_members'), $name ); ?>"><?php _e('Delete', 'feup_members'); ?></a>
								<?php }
						...
						<td class="desc">
							...
							if ( is_array( $role_2->capabilities ) ) {
								...
								if ( 1 < $cap_count ) printf( __('%1$s Capabilities', 'feup_members'), $cap_count );
								elseif ( 1 == $cap_count ) _e('1 Capability', 'feup_members');
							}
							else
								echo '<em>' . __('This role has no capabilities', 'feup_members') . '</em>'; ?>
							</p>
						</td><!-- .desc -->
			...
			<div class="tablenav">
				...
				<div class="alignleft actions">
					...
					<input type="submit" value="<?php _e('Apply', 'feup_members'); ?>" name="doaction2" id="doaction2" class="button-secondary action" />
				</div><!-- .alignleft .actions -->
