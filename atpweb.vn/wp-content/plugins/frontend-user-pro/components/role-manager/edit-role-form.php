...
$capabilities = feup_members_get_capabilities();

/* Check if the form has been submitted. */
if ( isset($_POST['edit-role-saved']) && 'Y' == $_POST['edit-role-saved'] ) {

	/* Verify the nonce. */
	check_admin_referer( feup_members_get_nonce( 'edit-roles' ) );
...
<div class="wrap">
	...
	<h2><?php printf(__('Edit the %1$s role', 'feup_members'), $role->name ); ?></h2>

	<?php if ( isset($role_updated) and $role_updated ) feup_members_admin_message( '', __('Role updated.', 'feup_members') ); ?>

	<?php do_action( 'feup_members_pre_edit_role_form' ); //Available pre-form hook for displaying messages. ?>
	...
	<div id="poststuff">
		...
		<form name="form0" method="post" action="<?php echo admin_url( esc_url( "users.php?page=roles&amp;action=edit&amp;role={$role->name}" ) ); ?>" style="border:none;background:transparent;">
			...
			<div class="postbox open">
				...
				<h3><?php printf( __('<strong>Role:</strong> %1$s', 'feup_members'), $role->name ); ?></h3>
				...
				<div class="inside">
					...
					<tr>
						...
						<td>
							<?php _e('Select which capabilities this role should have. Make sure you understand what the capability does before giving it to just any role. This is a powerful feature, but it can cause you some grief if you give regular ol\' Joe more capabilities than yourself.', 'feup_members'); ?>
							<br /><br />
						<?php
					...
					<tr>
						<th style="width: 20%;">
							<strong><?php _e('New Capabilities', 'feup_members'); ?></strong>
						</th>

						<td>
							<?php _e('Add up to six new capabilities with this form for this role (more can be added later). Please only use letters, numbers, and underscores.', 'feup_members'); ?>
							<br /><br />
			...
			<p class="submit" style="clear:both;">
				<input type="submit" name="Submit"  class="button-primary" value="<?php _e('Update Role', 'feup_members') ?>" />
