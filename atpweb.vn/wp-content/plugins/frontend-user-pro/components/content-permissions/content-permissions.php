 ...
 */
add_action( 'admin_menu', 'feup_members_content_permissions_create_meta_box' );

/* Saves the content permissions metabox data to a custom field. */
add_action( 'save_post', 'feup_members_content_permissions_save_meta', 1, 2 );

/* Add messages to the components form. */
add_action( 'feup_members_pre_components_form', 'feup_members_message_no_restrict_content' );
...
function feup_members_content_permissions_comments( $template ) {
	...
	if ( !empty( $roles ) && is_array( $roles ) ) {
		...
		$template = FEUP_MEMBERS_COMPONENTS . '/content-permissions/comments.php';
	}
	return $template;
}
 ...
 *
 * @since 0.1
 * @uses is_active_feup_members_component() Checks if the content_permissions component is active.
 * @uses feup_members_check_form_cap() Checks if the restrict_content capability has been given to a role.
 */
function feup_members_message_no_restrict_content() {
	if ( is_active_feup_members_component( 'content_permissions' ) && !feup_members_check_for_cap( 'restrict_content' ) ) {
		$message = __('No role currently has the <code>restrict_content</code> capability.  To use the <em>Content Permissions</em> component, at least one role must have this capability.', 'feup_members');
...
function feup_members_content_permissions_protect( $content ) {
	...
	if ( !empty( $roles ) && is_array( $roles ) ) {
		...
		$content = '<p class="restricted alert warning">' . __('Sorry, but you do not have permission to view this content.', 'feup_members') . '</p>';
	}
