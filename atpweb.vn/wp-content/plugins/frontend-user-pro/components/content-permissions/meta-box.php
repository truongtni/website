...
function feup_members_content_permissions_create_meta_box() {
	if ( current_user_can( 'restrict_content' ) ) {
		add_meta_box( 'content-permissions-meta-box', 'Content Permissions', 'feup_members_content_permissions_meta_box', 'post', 'advanced', 'high' );
		add_meta_box( 'content-permissions-meta-box', 'Content Permissions', 'feup_members_content_permissions_meta_box', 'page', 'advanced', 'high' );
	}
}
...
function feup_members_content_permissions_meta_box( $object, $box ) {
	...
	<p>
		<label for="roles"><?php _e('<strong>Roles:</strong> Restrict the content to these roles on the front end of the site.  If all boxes are left unchecked, everyone can view the content.', 'feup_members'); ?></label>
	</p>
