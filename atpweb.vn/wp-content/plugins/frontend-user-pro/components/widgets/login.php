...
class Feup_Members_Widget_Login extends WP_Widget {
	...
	function Feup_Members_Widget_Login() {
		...
		$control_ops = array( 'width' => 700, 'height' => 350, 'id_base' => 'feup_members-widget-login' );
		$this->WP_Widget( 'feup_members-widget-login', __('Login Form', 'widgets-reloaded'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array(
			...
			'username_label' => __('Username:', 'feup_members'),
			'password_label' => __('Password', 'feup_members'),
			'submit_text' => __('Log In', 'feup_members'),
			'remember_text' => __('Remember me?', 'feup_members'),
			'show_avatar' => true,
			'logged_out_text' => __('Please log into the site.', 'feup_members'),
			'logged_in_text' => __('You are currently logged in.', 'feup_members')
		);
