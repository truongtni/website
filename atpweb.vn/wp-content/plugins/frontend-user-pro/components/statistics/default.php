...
add_action( 'admin_menu', 'feup_members_component_add_stats_page' );

/* Additional capabilities required by the component. */
add_filter( 'feup_members_get_capabilities', 'feup_members_component_stats_capabilities' );
 ...
 * @since 0.2
 */
function feup_members_component_add_stats_page() {
	global $feup_members;

	$feup_members->stats_page = add_submenu_page( 'users.php', __( 'Feup_Members Statistics', 'feup_members' ), __( 'Feup_Members Stats', 'feup_members' ), 'view_stats', 'stats', 'feup_members_component_stats_page' );
}
 ...
 * @since 0.2
 */
function feup_members_component_stats_page() {

	$stats = get_option( 'feup_members_statistics' );

	if ( empty( $stats ) )
		feup_members_component_stats_create_initial_stats();

	require_once( FEUP_MEMBERS_COMPONENTS . '/statistics/statistics.php' );
}
 ...
 * @since 0.2
 */
function feup_members_component_stats_capabilities( $capabilities ) {
...
function update_stats_package( $user_id ) {
	...
	update_option( 'feup_members_statistics', $stats );
}
 ...
 * @since 0.2
 */
function feup_members_component_stats_create_initial_stats() {
	global $wp_roles;
	...
	add_option( 'feup_members_statistics', $stats );
