 ...
 * @since 0.2.0
 */
function feup_members_get_setting( $option = '' ) {
	global $feup_members;

	if ( !$option )
		return false;

	if ( !isset( $feup_members->settings ) )
		$feup_members->settings = get_option( 'feup_members_settings' );

	if ( !is_array( $feup_members->settings ) || empty( $feup_members->settings[$option] ) )
		return false;

	return $feup_members->settings[$option];
}
 ...
 * @since 0.2.0
 */
function feup_members_count_roles() {
	global $wp_roles;
 ...
 * @since 0.2.0
 */
function feup_members_get_active_roles() {
	global $wp_roles, $feup_members;

	if ( !isset( $feup_members->active_roles ) ) {

		$active = array();

		foreach ( apply_filters( 'editable_roles', $wp_roles->role_names ) as $role => $name ) {

			$count = feup_members_get_role_user_count( $role );

			if ( !empty( $count ) )
				$active[$role] = $name;
		}

		$feup_members->active_roles = $active;
	}

	return $feup_members->active_roles;
}
 ...
 * @since 0.2.0
 */
function feup_members_get_inactive_roles() {
	global $wp_roles, $feup_members;

	if ( !isset( $feup_members->inactive_roles ) ) {
		...
		$feup_members->inactive_roles = $inactive;
	}

	return $feup_members->inactive_roles;
}
 ...
 */
function feup_members_get_role_user_count( $user_role = '' ) {
	global $feup_members;

	/* If the count is not already set for all roles, let's get it. */
	if ( !isset( $feup_members->role_user_count ) ) {
		...
		$feup_members->role_user_count = $avail_roles;
	}

	/* If the $user_role parameter wasn't passed into this function, return the array of user counts. */
	if ( empty( $user_role ) )
		return $feup_members->role_user_count;

	/* If the role has no users, we need to set it to '0'. */
	if ( !isset( $feup_members->role_user_count[$user_role] ) )
		$feup_members->role_user_count[$user_role] = 0;

	/* Return the user count for the given role. */
	return $feup_members->role_user_count[$user_role];
}

/**
 * Function for listing users like the WordPress function currently uses for authors.
 *
 * Eventually, I hope to remove this function in reador of wp_list_users():
 * @link http://core.trac.wordpress.org/ticket/15145
 *
 * @since 0.1.0
 * @uses get_users()
 */
function feup_members_list_users( $args = array() ) {
	...
	if ( !empty( $users ) ) {
		...
		$output .= '<ul class="xoxo feup_members-list-users">';
	...
	$output = apply_filters( 'feup_members_list_users', $output );
