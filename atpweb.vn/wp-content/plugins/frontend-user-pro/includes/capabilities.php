...
add_filter( 'feup_members_get_capabilities', 'feup_members_remove_old_levels' );
 ...
 *
 * @since 0.1.0
 * @uses feup_members_get_default_capabilities() Gets an array of WP's default capabilities.
 * @uses feup_members_get_role_capabilities() Gets an array of all the capabilities currently mapped to a role.
 * @uses feup_members_get_additional_capabilities() Gets an array of capabilities added by the plugin.
 * @return array $capabilities An array containing all of the capabilities.
 */
function feup_members_get_capabilities() {

	/* Capabilities array. */
	$capabilities = array();

	/* Grab the default capabilities (these are set by the plugin so the user doesn't lose them). */
	$default_caps = feup_members_get_default_capabilities();

	/* Get the user capabilities that are already set. */
	$role_caps = feup_members_get_role_capabilities();

	/* Gets capabilities added by the plugin. */
	$plugin_caps = feup_members_get_additional_capabilities();
	...
	$capabilities = apply_filters( 'feup_members_get_capabilities', $capabilities );
 ...
 * folks accidentally deleting the default WordPress capabilities, the feup_members_default_capabilities() will 
 ...
 */
function feup_members_get_role_capabilities() {
	global $wp_roles;
...
/**
 * Additional capabilities provided by the Feup_Members plugin that gives users permissions to handle certain features
 * of the plugin.
 *
 * @todo Integrate 'edit_roles' into the settings.  It should be a priority on initial setup.
 * @todo Move each capability within its component. Use the 'feup_members_get_capabilities' filter hook to add them.
 *
 * @since 0.1.0
 * @return array $capabilities
 */
function feup_members_get_additional_capabilities() {
 ...
 */
function feup_members_get_default_capabilities() {
 ...
 *
 * @since 0.1.0
 * @uses feup_members_get_role_capabilities() Checks for capability in array of role caps.
 ...
 */
function feup_members_check_for_cap( $cap = '' ) {
	...
	$caps = feup_members_get_role_capabilities();
 ...
 */
function feup_members_get_old_levels() {
 ...
 * remove_filter( 'feup_members_get_capabilities', 'feup_members_remove_old_levels' );
 *
 * @since 0.1.0
 ...
 */
function feup_members_remove_old_levels( $capabilities ) {
	return array_diff( $capabilities, feup_members_get_old_levels() );
}
