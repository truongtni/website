...
class Feup_Members_Widget_Login extends WP_Widget {
	 ...
	 * @since 0.1.0
	 */
	function Feup_Members_Widget_Login() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'feup_members-widget-login', esc_attr__( 'Login Form', 'feup_members' ), $widget_options, $control_options );
	}
	...
	function widget( $args, $instance ) {
		...
		if ( is_user_logged_in() ) {
			...
			if ( !empty( $logged_in_text ) )
				echo do_shortcode( shortcode_unautop( wpfeutop( $logged_in_text ) ) );
		}
		...
		/* If the current user is not logged in. */
			...
			if ( $logged_out_text )
				echo do_shortcode( shortcode_unautop( wpfeutop( $logged_out_text ) ) );

			/* Output the login form. */
			echo '<div class="feup_members-login-form">' . wp_login_form( $args ) . '</div>';
		}
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => esc_attr__( 'Log In', 'feup_members' ), 
			'label_username' => esc_attr__( 'Username', 'feup_members' ),
			'label_password' => esc_attr__( 'Password', 'feup_members' ),
			'label_log_in' => esc_attr__( 'Log In', 'feup_members' ),
			'label_remember' => esc_attr__('Remember Me', 'feup_members' ),
			'form_id' => 'loginform',
			...
			'value_username' => '',
			'show_avatar' => true,
			'logged_out_text' => esc_html__( 'Please log into the site.', 'feup_members' ),
			'logged_in_text' => esc_html__( 'You are currently logged in.', 'feup_members' )
		);
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'feup_members' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'remember' ); ?>"><?php _e( '"Remember me" checkbox?', 'feup_members' ); ?> <code>remember</code></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'value_remember' ); ?>"><?php _e( 'Check "remember me"?', 'feup_members' ); ?> <code>value_remember</code></label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'logged_out_text' ); ?>"><?php _e( 'Logged out text:', 'feup_members' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'logged_in_text' ); ?>"><?php _e( 'Logged in text:', 'feup_members' ); ?></label>
