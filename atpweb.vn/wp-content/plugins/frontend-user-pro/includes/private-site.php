...
add_action( 'template_redirect', 'feup_members_please_log_in', 1 );

/* Disable content in feeds if the feed should be private. */
add_filter( 'the_content_feed', 'feup_members_private_feed' );
add_filter( 'the_excerpt_rss', 'feup_members_private_feed' );
add_filter( 'comment_text_rss', 'feup_members_private_feed' );
 ...
 */
function feup_members_please_log_in() {

	/* Check if the private blog feature is active. */
	if ( feup_members_get_setting( 'private_blog' ) ) {
 ...
 */
function feup_members_private_feed( $content ) {

	if ( feup_members_get_setting( 'private_feed' ) )
		$content = feup_members_get_setting( 'private_feed_error' );
