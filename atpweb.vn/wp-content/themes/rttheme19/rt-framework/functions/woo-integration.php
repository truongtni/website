...
if ( ! function_exists( 'rt_woocommerce_template_loop_product_thumbnail' ) ) {
	...
	function rt_woocommerce_template_loop_product_thumbnail() {
		...
		if( ! empty( $thumbnail_image_output ) ) {
			...
			//echo '</div>'; 
		?>
<div align="center">        <a href="<?=LINKWEB?>/w/?theme=<?=get_the_id()?>"><?php the_post_thumbnail("medium",array( "title" => get_the_title(),"alt" => get_the_title() ));?></a>
<p class="button"><a href="<?=LINKWEB?>/w/?theme=<?=get_the_id()?>">Demo</a><a href="<?php the_permalink() ;?>"><?=BIZHOST_BUTTON_CREATE_WEBSITE?></a></p>
<?=get_the_title();?>
</div>
...
if ( ! function_exists( 'woocommerce_taxonomy_archive_description' ) ){
	...
	function rt_get_product_search_form() {
		$form = '<form role="search" method="get" id="searchform" class="rt_form" action="' . esc_url( home_url( '/'  ) ) . '">
			<div>
				<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . __( 'Nhập từ khóa', 'woocommerce' ) . '" />
