...
function rt_get_attachment_id_from_src ($image_src) { 
	...
	$image_src_https = str_replace("http://", "https://", $image_src);
	$image_src_http = str_replace("https://", "http://", $image_src);

	$query = "SELECT ID FROM {$wpdb->posts} WHERE guid='$image_src_http' OR guid='$image_src_https'";
