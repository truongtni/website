...
<?php if ( have_comments() ) : ?>
			...
			<h6 id="comments-title"><?php echo (get_comments_number() == 1) ? __('One Comment' , 'rt_theme') : sprintf( __('%s Comments' , 'rt_theme') , get_comments_number() ); ?></h6>								
...
$fields =  array(
	'author' => '<li class="comment-form-author">'.
	            '<input id="author" name="author" type="text" placeholder="' . __('Name','rt_theme') . ( $req ? ' *' : '' ) . '" value="' . $commnet_author . '" size="30"' . $aria_req . ' />'.
	            '</li>',

	'email' => '<li class="comment-form-email">'.
	            '<input id="email" name="email" type="text" placeholder="' . __('Email','rt_theme') . ( $req ? ' *' : '' ) . '" value="' . $commnet_author_email . '" size="30"' . $aria_req . ' />'.
	            '</li>',


	'url' => '<li class="comment-form-url ">'.
	            '<input id="url" name="url" type="text" placeholder="' . __('Website','rt_theme') . '" value="' . $comment_author_url . '" size="30" />'.
	            '</li>',
...
$comments_args = array( 	
	'comment_field'        => '<div class="text-boxes"><ul><li><textarea tabindex="4" class="comment_textarea" rows="10" id="comment" name="comment" placeholder="'. __('Comment','rt_theme') .' *"></textarea></li></ul></div>',
	'id_form'              => 'commentform', 
